...
- [Kuesioner Apple-to-Apple](#kuesioner-apple-to-apple)
...
3. **Membangun** aplikasi web interaktif berbasis Flask yang memungkinkan pengguna mengetahui tipe kepribadiannya melalui kuesioner 7 pertanyaan yang langsung sesuai dengan fitur dataset (apple-to-apple).
...
- Menyediakan referensi implementasi kuesioner yang langsung sesuai dengan fitur dataset (apple-to-apple) untuk perbandingan algoritma machine learning yang adil.
...
- Kuesioner menggunakan 7 pertanyaan yang langsung sesuai dengan fitur dataset untuk memastikan perbandingan apple-to-apple antar algoritma.
...
| No | Fitur | Deskripsi |
|----|-------|-----------|
| 1 | **Kuesioner Interaktif** | 7 pertanyaan yang langsung sesuai dengan fitur dataset (apple-to-apple) dengan progress bar real-time, terdiri dari 5 pertanyaan slider numerik dan 2 pertanyaan biner (Ya/Tidak) |
...
```
Pengguna ──> Form Kuesioner ──> 7 jawaban (= 7 fitur dataset langsung)
                                        │
                                        │  (Apple-to-apple: input = fitur dataset)
                                        │
                                        v
                              ┌─────────────────────┐
                              │  Feature Scaling     │
...
│   ├── Kuesioner.html            # Halaman kuesioner 7 pertanyaan apple-to-apple (progress bar)
...
│ (/login) │     │  (/)        │     │  (/analyze)  │     │  (/hasil)         │
│          │     │             │     │  POST Form   │     │                   │
│ POST JSON│     │ 7 soal      │     │ Simpan ke    │     │ Prediksi +        │
│ via AJAX │     │ = fitur     │     │ session      │     │ Metrik + Chart.js │
└──────────┘     └─────────────┘     └──────────────┘     └───────────────────┘
      │                                                            │
      │           ┌─────────────┐     ┌──────────────┐             │
      │           │   Tentang   │     │  Pengaturan  │             │
...
| 2 | **Kuesioner** | `/` | GET | Halaman utama. Menampilkan 7 pertanyaan yang langsung sesuai fitur dataset (apple-to-apple) dengan progress bar. Memerlukan login. |
...
## Kuesioner Apple-to-Apple

Untuk memastikan perbandingan antar algoritma bersifat **apple-to-apple**, kuesioner dirancang agar **setiap pertanyaan langsung sesuai dengan satu fitur dataset**. Tidak ada proses transformasi atau feature engineering tambahan — jawaban pengguna langsung menjadi input model, identik dengan format data yang digunakan saat training.

### Prinsip Apple-to-Apple

1. **Kuesioner = Fitur Dataset**: 7 pertanyaan = 7 fitur dataset. Tidak ada konversi atau pemetaan.
2. **Jawaban = Input Model**: Nilai yang dimasukkan pengguna langsung dikirim ke model setelah scaling.
3. **Data Sama untuk Semua Algoritma**: Ketiga algoritma (KNN, SVM, Naive Bayes) menerima input yang **identik** — kuesioner, jawaban, fitur, dan scaling yang sama persis.

### Daftar Pertanyaan

| No | Pertanyaan | Fitur Dataset | Tipe Input | Rentang |
|----|-----------|---------------|------------|---------|
| Q1 | Berapa jam waktu yang Anda habiskan sendirian per hari? | `Time_spent_Alone` | Slider | 0 - 10 |
| Q2 | Apakah Anda memiliki ketakutan tampil di depan umum (demam panggung)? | `Stage_fear` | Ya/Tidak | 0 atau 1 |
| Q3 | Berapa kali Anda menghadiri acara sosial per bulan? | `Social_event_attendance` | Slider | 0 - 10 |
| Q4 | Berapa kali Anda pergi keluar rumah untuk bersosialisasi per minggu? | `Going_outside` | Slider | 0 - 10 |
| Q5 | Apakah Anda merasa lelah/terkuras setelah bersosialisasi? | `Drained_after_socializing` | Ya/Tidak | 0 atau 1 |
| Q6 | Berapa jumlah teman dekat dalam lingkaran pertemanan Anda? | `Friends_circle_size` | Slider | 0 - 15 |
| Q7 | Berapa kali Anda memposting di media sosial per minggu? | `Post_frequency` | Slider | 0 - 10 |

### Keunggulan Pendekatan Apple-to-Apple

- **Tidak ada feature engineering tambahan**: Jawaban langsung = fitur model, sehingga tidak ada distorsi dari transformasi.
- **Perbandingan yang adil**: Karena input identik, perbedaan hasil murni disebabkan oleh perbedaan algoritma, bukan perbedaan input.
- **Transparansi**: Pengguna dapat melihat bahwa pertanyaan kuesioner = fitur dataset pada halaman hasil analisis.
...
```
Jawaban Kuesioner (= Fitur Dataset langsung):
    Time_spent_Alone    = 2
    Stage_fear          = 0 (Tidak)
    Social_event_att.   = 7
    Going_outside       = 6
    Drained_after_soc.  = 0 (Tidak)
...
1. **Feature Scaling**: 7 jawaban kuesioner (= 7 fitur dataset) langsung dinormalisasi menggunakan `StandardScaler` yang **sama** dengan yang digunakan saat training. File `scaler.pkl` menyimpan parameter $\mu$ dan $\sigma$ dari data training. Tidak ada feature engineering tambahan karena kuesioner sudah apple-to-apple dengan fitur dataset.

2. **Prediksi Multi-Model**: Ketiga model memprediksi secara **independen** dengan **input identik**. Setiap model menghasilkan:
   - **Kelas prediksi**: Extrovert (0) atau Introvert (1)
   - **Probabilitas**: Distribusi keyakinan untuk setiap kelas

3. **Majority Voting**: Prediksi akhir ditentukan oleh suara terbanyak:
...
- **Isi**: 7 nilai fitur yang langsung diisi dari jawaban kuesioner pengguna (apple-to-apple)
...
| File | Deskripsi |
|------|-----------|
| `app.py` | File utama aplikasi Flask. Berisi: (1) Load model `.pkl` saat startup, (2) Fungsi `predict_from_answers()` yang menerima 7 jawaban kuesioner langsung sebagai fitur dataset (apple-to-apple) dan melakukan prediksi, (3) Route definitions untuk semua halaman. Model di-load **sekali** saat startup, bukan per-request, untuk efisiensi. |
...
| `Kuesioner.html` | 7 pertanyaan yang di-generate via JavaScript, langsung sesuai dengan fitur dataset (apple-to-apple). Terdiri dari 5 pertanyaan slider numerik dan 2 pertanyaan biner (Ya/Tidak). Progress bar real-time menunjukkan jumlah pertanyaan yang dijawab. Tombol Submit baru aktif setelah semua 7 pertanyaan dijawab. |
...
4. **Aplikasi web berhasil dibangun** menggunakan Flask dan Chart.js yang mampu memprediksi kepribadian pengguna secara real-time berdasarkan kuesioner 7 pertanyaan yang langsung sesuai fitur dataset (apple-to-apple), dengan sistem voting mayoritas dari 3 algoritma untuk hasil yang lebih robust.
...
4. **Ekspansi fitur**: Menambahkan fitur perilaku baru ke dataset dan kuesioner untuk meningkatkan akurasi klasifikasi kepribadian.
