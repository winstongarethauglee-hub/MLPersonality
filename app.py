...
def predict_from_answers(answers):
    """Predict personality from 7 questionnaire answers that directly match dataset features."""
    time_alone = float(answers[0])         # 0-10
    stage_fear = int(answers[1])           # 0 or 1
    social_event = float(answers[2])       # 0-10
    going_outside = float(answers[3])      # 0-10
    drained = int(answers[4])              # 0 or 1
    friends_circle = float(answers[5])     # 0-15
    post_freq = float(answers[6])          # 0-10
...
def analyze():
    ...
    for i in range(1, 8):
        ...
        answers.append(float(val) if val else 0)
...
def hasil():
    ...
    try:
        answers = session.get('answers', [0] * 7)
